...
document.getElementById('loginSubmitBtn').addEventListener('click', () => {
  ...
  const acc = window.db.accounts.find(a => a.email === em && a.password === pw);
  if (!acc)          { showToast('Invalid email or password.', 'error'); return; }
  if (!acc.verified) { showToast('Please verify your email first.', 'error'); return; }

  const token = btoa(JSON.stringify({ email: em, role: acc.role, time: Date.now() }));
  localStorage.setItem('auth_token', token);
  setAuthState(true, acc);
  showToast(`Welcome back, ${acc.firstName}!`, 'success');
  // Small delay lets the DOM/auth state settle before routing
...
if (savedToken) {
  try {
    ...
    const acc = window.db.accounts.find(a => a.email === decoded.email);
